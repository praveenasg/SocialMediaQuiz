...
export type KeyMirrorRecursive<O> = $ObjMapi<O, (<O: {...}>(mixed, O) => KeyMirrorRecursive<O>) & (<K>(K) => K)>;
export type KeyMirrorRecursiveWithPrefix<O> = $ObjMapi<O, (<O: {...}>(mixed, O) => KeyMirrorRecursiveWithPrefix<O>) & ((mixed) => string)>;
type keyMirrorRecursiveFn = (<O: {...}>(obj: O, _: void) => KeyMirrorRecursive<O>) & (<O: {...}>(obj: O, prefix: string) => KeyMirrorRecursiveWithPrefix<O>);
/**
...
const keyMirrorRecursive: keyMirrorRecursiveFn = (obj: {...}, prefix) => {
  const ret: {
    [string]: KeyMirrorRecursiveWithPrefix<$FlowFixMe> | string
  } = {};
